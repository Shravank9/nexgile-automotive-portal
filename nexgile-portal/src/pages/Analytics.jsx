      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
        {[
          ...
          <div key={k.l} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:'18px 20px'}}>
            ...
            <div style={{fontFamily:'var(--syne)',fontWeight:800,fontSize:26,color:k.c,lineHeight:1}}>{k.v}</div>
                  ...
                  <tr key={a.name}>
                    <td>
                      <div style={{display:'flex',alignItems:'center',gap:10}}>
                        <div style={{width:32,height:32,borderRadius:'50%',background:['#e8a020','#3b82f6','#22c55e','#a855f7'][i],display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--syne)',fontWeight:700,fontSize:12,color:'#000',flexShrink:0}}>
                    ...
                    <td>{a.leads}</td>
                    <td style={{fontFamily:'var(--syne)',fontWeight:700}}>{a.closed}</td>
                    <td><span style={{color:conv>=35?'var(--green)':conv>=25?'var(--accent)':'var(--red)',fontWeight:600}}>{conv}%</span></td>
                    <td style={{fontFamily:'var(--syne)',fontWeight:700,color:'var(--accent)'}}>${(a.revenue/1000).toFixed(0)}K</td>
        ...
        <div style={{padding:20,display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
          {FORECASTS.map(f=>(
            <div key={f.month} style={{background:'var(--bg3)',border:'1px solid var(--border)',borderRadius:10,padding:20,textAlign:'center'}}>
              ...
              <div style={{fontFamily:'var(--syne)',fontWeight:800,fontSize:34,color:'var(--accent)',lineHeight:1}}>{f.predicted}</div>
