  ...
  return (
    <div className="page-fade">
      ...
      {counts['Follow-up Due']>0&&(
        <div style={{background:'rgba(232,160,32,0.08)',border:'1px solid rgba(232,160,32,0.2)',borderRadius:10,padding:'10px 16px',marginBottom:16,display:'flex',alignItems:'center',justifyContent:'space-between',fontSize:13}}>
                    ...
                    <tr key={l.id} style={{cursor:'pointer'}} onClick={()=>setDetail(l)}>
                      <td>
                        <div style={{display:'flex',alignItems:'center',gap:10}}>
                          <div style={{width:34,height:34,borderRadius:'50%',background:AVATARS[i%AVATARS.length],display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--syne)',fontWeight:700,fontSize:12,color:'#000',flexShrink:0}}>
                      ...
                      <td style={{fontFamily:'var(--syne)',fontWeight:600,fontSize:13}}>{l.budget}</td>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setDetail(null)}>
          <div className="modal-box" style={{maxWidth:480}}>
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:20}}>
              <div style={{width:48,height:48,borderRadius:'50%',background:AVATARS[data.findIndex(l=>l.id===detail.id)%AVATARS.length],display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'var(--syne)',fontWeight:800,fontSize:16,color:'#000',flexShrink:0}}>
                {detail.name.split(' ').map(w=>w[0]).join('').slice(0,2)}
              </div>
              <div style={{flex:1}}>
                <div style={{fontFamily:'var(--syne)',fontWeight:800,fontSize:18}}>{detail.name}</div>
