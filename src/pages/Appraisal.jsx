            ...
            {calcDone && est && (
              <div style={{ marginTop: 18, border: '1px solid rgba(34,197,94,0.2)', borderRadius: 10, overflow: 'hidden' }}>
                <div style={{ background: 'rgba(34,197,94,0.06)', padding: '14px 16px', borderBottom: '1px solid rgba(34,197,94,0.1)' }}>
                  ...
                  <div style={{ fontFamily: 'var(--font)', fontWeight: 800, fontSize: 30, color: 'var(--green)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
                  {[['KBB Estimate', est.kbb, 'var(--blue)'], ['Black Book', est.bb, 'var(--accent)']].map(([l, v, c]) => (
                    <div key={l} style={{ padding: '12px 16px', borderRight: l === 'KBB Estimate' ? '1px solid var(--border)' : 'none' }}>
                      ...
                      <div style={{ fontFamily: 'var(--font)', fontWeight: 700, fontSize: 18, color: c }}>₹{v.toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {data.map(a => (
              <div key={a.id} className="mini-list-item">
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  {a.offerMade > 0
                    ? <div style={{ fontFamily: 'var(--font)', fontWeight: 700, fontSize: 15, color: 'var(--green)' }}>₹{a.offerMade.toLocaleString()}</div>
              ...
              {filtered.map(a => {
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td style={{ color: 'var(--blue)', fontFamily: 'var(--font)', fontWeight: 600 }}>₹{a.kbbValue.toLocaleString()}</td>
                    <td style={{ color: 'var(--accent)', fontFamily: 'var(--font)', fontWeight: 600 }}>₹{a.bbValue.toLocaleString()}</td>
                    <td style={{ fontFamily: 'var(--font)', fontWeight: 700, color: a.offerMade > 0 ? 'var(--green)' : 'var(--muted)' }}>
                      {a.offerMade > 0 ? '₹' + a.offerMade.toLocaleString() : '—'}
                    </td>
                    <td style={{ color: 'var(--red)' }}>₹{a.reconCost.toLocaleString()}</td>
                    <td style={{ fontFamily: 'var(--font)', fontWeight: 700, color: net > 0 ? 'var(--green)' : 'var(--red)' }}>
